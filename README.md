...
- 🎭 **N-gram 混合模型 (Blended Model)** *(Session 9 ✅ COMPLETE)*：多語料庫加權合併，兼顧正式與口語
  - **混合策略**：70% rime-essay (正式/書面) + 30% PTT-Corpus (聊天/口語)
  - **參數優化**：threshold=2, topk=40（經實驗驗證的最佳平衡點）
  - **檔案大小**：**1.64MB**（v1.1 正式版，適中且高效）
  - **多樣性**：18,426 unigrams, 116,672 bigrams (2.7x more than v1.0!)
  - **品質提升**：**+9.3% 整體改善**（正式寫作 +7.8%，聊天 +12.2%，混合 +8.3%）
  - **資料來源**：rime-essay (376K entries) + PTT八卦語料庫 (418K lines, 10.7M chars)
  - **處理流程**：串流處理 → 加權合併 → 智慧剪枝（保留 top 40 而非 top 10）
  - **實驗驗證**：測試 3 個版本（v1.0/v1.1/v1.2），v1.1 為最佳性價比
  - **Chrome Extension 就緒**：< 2MB，載入快速，遠低於 5MB 限制 ⚡
  - **生產就緒**：v1.1 已部署至 MVP1 (`ngram_blended.json`)，可直接使用 ✅
