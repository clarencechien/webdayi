...
**[立即試用 WebDaYi MVP1 v8 →](https://clarencechien.github.io/webdayi/)** (GitHub Pages)

在瀏覽器中體驗核心大易輸入引擎，具備進階功能：
- ✨ **自動複製** *(v8 NEW!)*：選字後自動複製到剪貼簿，無需手動點擊！
- 🗑️ **清除按鈕** *(v8 NEW!)*：一鍵清空輸出緩衝區！
...
**目前階段**：✅ MVP 1.0 v8 完成！
**完成度**：~52%（Phase 0 與 MVP 1 v8 完成，MVP 2a 下一步）
...
**最新成就**：MVP 1.0 v8 具備自動複製 + 清除按鈕（59/59 測試通過）
- ✨ **自動複製**：選字後自動複製到剪貼簿，無需手動點擊！（MVP1.11）
- 🗑️ **清除按鈕**：一鍵清空輸出緩衝區（MVP1.12）
- 🔄 **可切換控制**：自動複製功能可隨時開啟/關閉
- ✅ **頻率轉換器 v2**：使用台灣教育部 2000 高頻字資料進行智慧排序
- 🐛 **Bug 已修復**：自動選字會遵循使用者偏好（v7）
...
**[啟動 WebDaYi MVP1 v8 →](https://clarencechien.github.io/webdayi/)**
...
- **新功能 (v7)**：使用 ◀ **上一頁** / **下一頁** ▶ 按鈕在手機/平板上輕鬆翻頁！
- **新功能 (v8)**：選字後**自動複製到剪貼簿**，無需手動點擊！（可切換開啟/關閉）
- **新功能 (v8)**：使用「清除」按鈕一鍵清空輸出緩衝區
...
**最後更新**：2025-11-10
**狀態**：MVP 1.0 v8 完成（自動複製 + 清除按鈕 + 頻率轉換器 v2）
**版本**：1.0.8-alpha（MVP1 v8 with auto-copy + clear button + Converter v2 with frequency-based ranking）
