...
| **選字鍵**(大易/Lite 傳統配置) | `空白`=第 1、`'`=第 2、`[`=第 3、`]`=第 4、`-`=第 5,`=`=換頁。**一頁固定 5 個,每一格都按得到**;數字鍵是大易碼,不當選字鍵 |
...
| 功能 | 說明 |
|---|---|
| **Mini 模式(A′ 版面)** | IME 式兩列:第 1 列=暫定字(含碼標)+ 固定 5 格候選 + 頁碼 + 模式標籤,第 2 列=輸出;整條可拖曳視窗(桌面 PWA 的 window-controls-overlay),**不放虛擬鍵盤**,與 Lite 相同。**連按兩下 `Ctrl` 切換**或 `Esc` 離開;右上角標籤點一下切中/英 |
| **版面穩定(A′)** | **候選區固定 5 格、寬度固定**,分隔線推到右邊,左區(句子)拿走其餘全部寬度。候選位置永遠不動——第 3 格永遠是 `[`;候選不足 5 個時留空位,版面不塌。待配對格也有固定坑位 |
...
node smart/tests/ui_smoke.js          # 真瀏覽器 UI 煙霧測試(60 項;需 npm i playwright)
...
同步逐鍵播放版面提案,可切換 1100px / 560px 視窗,按 `=` 兩條一起換頁。

**結論:採用 A′**(2026-08-12),已實作進 `/smart/`。比較內容保留備查:

| | 提案 | 差別 |
|---|---|---|
| **A** | 現況(對照組) | 左區固定 42%、格子下標碼、候選有多少塞多少 |
| **A′** | 改版 | 格子與碼標**與 A 相同**;差別在版面:**分隔線右推**讓候選區固定 5 格、左區拿走其餘全部寬度、`=` 換頁並顯示頁碼 |

實測(1100px):A 的候選會塞到 12 個,但選字鍵只有 6 個 —— **第 7 個以後根本按不到**,
而且起點隨字數浮動;A′ 固定 5 格、位置不動且每格都按得到,左區可見字數從 10 → **21**。
本句 8 個字的候選數是 10/19/4/7/12/2/19/7,一頁 5 個 → 多數 2 頁內翻完,最多的「設」要 4 頁;
常用字幾乎都在第 1 頁,翻頁是冷僻字的逃生梯。

第 1 版的 B(右靠齊)/ C(Inline composition)/ D(聚焦)收在頁面下方的摺疊區備查。
