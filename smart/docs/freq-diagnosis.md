...
**結論:已定罪並修正。修正後 2 碼 top-1(有左 context)= 95.7%,過 85% 門檻。**
...
3. **修剪長尾**:2 字詞 count≥2、3 字詞 ≥3、4 字詞 ≥5。
   word_db.json 16.6MB → **3.8MB**;char_bigram.json → **1.2MB**。
...
|---|---|---|---|
| 2 碼 top-1(有左 context) | ~91%(小樣本 57 字;實際體感更差,見附註) | **95.7%** | ≥ 85% ✅ |
| 2 碼 top-3(有左 context) | 未支援(無候選排序介面) | **95.5%** | ≥ 95% ✅ |
| 2 碼 top-1(句首無 context) | — | 97.5% | 報告即可 |
| 整句全對率 | — | 77.4% | — |
| 4 碼(傳統)top-1 | — | 97.2% | ~100%(見附註 2)|
| 整句解碼延遲(159 句平均) | — | 2.5ms / 句(每鍵 ≪ 10ms) | < 10ms ✅ |
...
1. **同 2 碼前綴的高頻近義競爭**(證/這、設/說、塞/賽):字 bigram glue
   資訊量不足以區分,屬 2 碼模式的固有誤差,靠修正 UI 一按替換(top-3 95.5% 保證)。
2. **詞邊界切分歧義**(存檔→在機):需要更強的跨詞模型;可等使用者習慣層
   (μ)上線後由個人資料補強。
3. **句首無 context 的專名**(票、藥):預期內,handoff 不設門檻。

**若要再往上推**:引入真正語料級的字 bigram(zh-tw wiki + 台灣新聞語料),
但目前 95.7% 已遠過門檻,PoC 階段不做(YAGNI)。
